...
class FaceRecognition():
    ...
    def __init__(self):
        self.predictor_path = "/data/alinhador/shape_predictor_68_face_landmarks.dat"  # http://dlib.net/files/shape_predictor_68_face_landmarks.dat.bz2
        self.face_rec_model_path = "/data/alinhador/dlib_face_recognition_resnet_model_v1.dat"  # http://dlib.net/files/dlib_face_recognition_resnet_model_v1.dat.bz2
        
        #self.detector = dlib.cnn_face_detection_model_v1("/data/alinhador/mmod_human_face_detector.dat")
    ...
    def comparar_pessoas_google_imagens(self, img_bbox, nome_pessoa):
        ...
        database = pk.load(open("/data/alinhador/database_g_image.pk", "rb"))
    ...
    def comparar_pessoas(self, img_bbox, nome_pessoa):
        database = {}  #armazena as imagens da pasta do nome da pessoa
        nome_pessoa = nome_pessoa.replace(" ", "_")
        for f in glob.glob("/data/alinhador/faceDB/lfw/" + nome_pessoa + "/*.jpg"):
            img = io.imread(f)
            label = f.split("/")[-1]
            detections = self.detect_hog(img)
    ...
    def criar_db_g_images(self, folder):
        ...
        for f in glob.glob(folder + "*.jpg"):
            ...
            if len(detections) == 1:  # se houver apenas uma face na imagem
                ...
                embedding = self.get_embedding(detection, img)
                database[embedding] = label
        pk.dump(database, open("/data/alinhador/database_g_image.pk", "wb"))
